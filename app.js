(() => {
  ...
  function textItemToPosition(item, viewport, pageNumber) {
    ...
    const width = Math.max(height * 0.55, Math.abs(item.width * state.scale));
    return {
      page: pageNumber,
      raw: item.str.trim(),
      ...
      width,
      height,
      centerX: tx[4] + width / 2,
      centerY: tx[5] - height / 2,
    };
  }
      ...
      if (candidate) {
        ...
        if (page) {
          drawMarker(
            record,
            candidate.page,
            candidate.centerX / page.viewport.width,
            candidate.centerY / page.viewport.height,
            "auto",
            false
          );
          autoMatched += 1;
  ...
  function drawMarker(record, pageNumber, xRatio, yRatio, kind, manual) {
