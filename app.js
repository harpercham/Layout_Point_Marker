(() => {
  ...
  async function renderPdf() {
    ...
    for (let pageNumber = 1; pageNumber <= state.pdfDoc.numPages; pageNumber += 1) {
      ...
      state.pages.push({ pageNumber, viewport, stage, markerLayer, candidates });
    }
  ...
  function textItemToPosition(item, viewport, pageNumber) {
    ...
    const textLength = Math.max(fontHeight * 0.55, Math.abs(item.width * state.scale));

    const directionLength = Math.max(0.0001, Math.hypot(tx[0], tx[1]));
    const dirX = tx[0] / directionLength;
    const dirY = tx[1] / directionLength;

    const startX = tx[4];
    const startY = tx[5];
    const endX = startX + dirX * textLength;
    const endY = startY + dirY * textLength;

    const isVertical = Math.abs(dirY) > Math.abs(dirX) * 0.7;

    // Keep a practical bounding box for matching, plus the real baseline endpoints
    // for positioning the marker after the point number.
    const left = isVertical
      ? Math.min(startX, endX) - fontHeight / 2
      : Math.min(startX, endX);
    const right = isVertical
      ? Math.max(startX, endX) + fontHeight / 2
    ...
    for (const line of lines) {
      ...
      for (let start = 0; start < line.items.length; start += 1) {
        ...
        for (let size = 1; size <= 5 && start + size - 1 < line.items.length; size += 1) {
          ...
          if (norm.length >= 4) {
            const firstItem = line.items[start];
            candidates.push({
              page: item.page,
              raw: joined,
              norm,
              ...
              startY: firstItem.startY,
              endX: item.endX,
              endY: item.endY,
              dirX: firstItem.dirX,
              dirY: firstItem.dirY,
              isVertical: firstItem.isVertical,
            });
          }
        }
      }
  ...
  function applyMarkers() {
    ...
    for (const [norm, record] of state.completed.entries()) {
      ...
      if (candidate) {
        ...
        if (page) {
          const anchor = getMarkerAnchorBelowPoint(candidate);
          drawMarker(
            record,
            candidate.page,
  ...
  function getMarkerAnchorBelowPoint(candidate) {
    const gap = Math.max(2, Math.min(7, Math.min(candidate.width, candidate.height) * 0.35));

    if (candidate.isVertical) {
      // Use the visually lower end of a rotated/vertical point number.
      const lowerIsEnd = candidate.endY >= candidate.startY;
      return {
        x: lowerIsEnd ? candidate.endX : candidate.startX,
        y: Math.max(candidate.startY, candidate.endY) + gap,
      };
    }

    return {
      x: candidate.centerX,
      y: (candidate.bottom ?? (candidate.top + candidate.height)) + gap,
    };
  }

  function drawMarker(record, pageNumber, xRatio, yRatio, kind, manual, belowPoint = false) {
    const page = state.pages.find((p) => p.pageNumber === pageNumber);
    if (!page) return;

    const group = document.createElement("div");
    group.className = `mark-group${belowPoint ? " below-point" : ""}`;
    ...
    marker.className = `marker ${statusClass}`;
    marker.textContent = "✓";
    markerRow.append(marker);

    const dateLabel = document.createElement("div");
    dateLabel.className = "completion-date";
    dateLabel.textContent = record.completionDateLabel || record.date || "Date?";
    group.append(markerRow, dateLabel);
