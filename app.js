(() => {
  ...
  function applyMarkers() {
    ...
    for (const [norm, record] of state.completed.entries()) {
      ...
      if (candidate) {
        ...
        if (page) {
          const anchor = resolveCircleAnchor(page, candidate);
          drawMarker(
            record,
            candidate.page,
  ...
  function drawMarker(record, pageNumber, xRatio, yRatio, kind, manual, circleRadius = null) {
    ...
    const badgeSize = circleRadius
      ? Math.max(24, Math.min(38, circleRadius * 1.55))
      : 30;
