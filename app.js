(() => {
  ...
  function resolveBelowPointAnchor(candidate) {
    const badgeSize = Math.max(24, Math.min(32, candidate.height * 3.2));
    const gap = Math.max(2, candidate.height * 0.28);

    return {
      x: candidate.centerX,
      // The status badge is centred below the point-number text box.
      y: candidate.top + candidate.height + gap + badgeSize / 2,
      badgeSize,
    };
  }
